  ...
  module QueryMethods
    module AnyOf
      def any_of(*queries)
        ...
        build_query(queries) { |arel_query| @scope.where(arel_query) }
      end

      def none_of(*queries)
        queries = hash_map_queries(queries)
        build_query(queries) { |arel_query| @scope.where.not(arel_query) }
      end
      ...
      def construct_query_mappings(queries) # rubocop:disable Metrics/AbcSize
        { joins: Set.new, references: Set.new, includes: Set.new, arel_query: nil }.tap do |query_map|
